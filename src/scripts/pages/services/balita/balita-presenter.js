...
class BalitaPresenter {
       ...
       hitungIMT(berat, tinggi) {
	       ...
	       if (imt < 18.5) {
		       status = "Kurus";
		       saran = "Anda termasuk kurus. Perbanyak makan makanan bergizi, konsultasikan ke faskes terdekat untuk pemeriksaan lebih lanjut.";
		       warna = "#7FB3D5";
	       } else if (imt >= 18.5 && imt < 25) {
		       status = "Normal";
		       saran = "IMT Anda normal. Pertahankan pola makan sehat dan rutin berolahraga.";
		       warna = "#82E0AA";
	       } else if (imt >= 25 && imt < 30) {
		       status = "Gemuk";
		       saran = "Anda termasuk gemuk. Kurangi makanan tinggi kalori, perbanyak aktivitas fisik, dan konsultasikan ke faskes jika perlu.";
		       warna = "#F8C471";
	       } else {
		       status = "Obesitas";
		       saran = "Anda termasuk obesitas. Segera konsultasikan ke faskes untuk penanganan lebih lanjut, perbaiki pola makan dan tingkatkan aktivitas fisik.";
		       warna = "#F1948A";
	       }
       ...
       hitungZScoreAnak({ jk, umur, bb, tb, caraUkur }) {
	      ...
	      function dapatkanLMS(json, kunci, jenisKelamin) {
	       ...
	       const zScoreBBU = lmsBBU ? hitungZScore(bb, lmsBBU.L, lmsBBU.M, lmsBBU.S) : null;
	       const zScoreTBU = lmsTBU ? hitungZScore(tb, lmsTBU.L, lmsTBU.M, lmsTBU.S) : null;
	       const zScoreBBTB = lmsBBTB ? hitungZScore(bb, lmsBBTB.L, lmsBBTB.M, lmsBBTB.S) : null;
	       const zScoreIMTU = lmsIMTU ? hitungZScore(imt, lmsIMTU.L, lmsIMTU.M, lmsIMTU.S) : null;
	       let statusBBU = "-";
	       if (zScoreBBU !== null) {
	       ...
	       if (zScoreBBTB !== null) {
		       ...
		       else if (zScoreBBTB > 1 && zScoreBBTB <= 2) statusBBTB = "Risiko Gizi Lebih";
	       ...
	       if (zScoreIMTU !== null) {
		       ...
		       else if (zScoreIMTU > 1 && zScoreIMTU <= 2) statusIMTU = "Risiko Gizi Lebih";
